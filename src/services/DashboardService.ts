...
export class DashboardService {
  /**
   * Obtém as estatísticas gerais do dashboard, incluindo faturamento, deltas,
   * gráfico mensal, decomposição de margem e produtos mais vendidos.
   */
  static async getDashboardStats(days?: number) {
    ...
    completedSales.forEach((sale) => {
      ...
      totalLaborAndProvisions +=
        (sellerWage + sale.finalPrice * investmentRate + sale.finalPrice * product.lossIndex) *
        quatity;
    ...
    const costsPercent = hasRevenue
      ? (totalAcquisitionAndDirectCosts + totalLaborAndProvisions) / totalRevenueCompleted
      : 0;
    ...
    const netRevenue = sales
      ...
      .reduce((acc, sale) => acc + sale.finalPrice * sale.quantity, 0);
    ...
    const previousGrossRevenue = previousSales.reduce(
      (acc, sale) => acc + sale.finalPrice * sale.quantity,
      0
    );
  ...
  /**
   * Obtém o histórico de evolução do preço médio diário de venda de um produto.
   */
  static async getPriceEvolution(productId: string, days?: number) {
    let dateFilter = {};
    if (days) {
      const startDate = new Date();
      startDate.setDate(startDate.getDate() - days);
    ...
    return Object.values(dailyPriceMap).map((day) => ({
      date: day.date,
